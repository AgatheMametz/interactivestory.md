...
- **Options** : bloc plus indenté ; **chaque ligne non vide** est du Markdown interprété comme le corps (`(if:)`, `(ifnot:)`, ternaires, `(set:)`, `once …`, etc.). La ligne doit finir par au moins un lien `[libellé](id_passage)` visible après évaluation ; la **première** paire lien / cible sert au menu.
...
- `(goto: id_passage)` — enchaîne sur un autre passage après l’affichage du bloc courant (corps ou segment `(wait:)`).
- `(wait: ms)` — pause en millisecondes entre deux morceaux du même passage (plusieurs `(wait:)` possibles). Ignoré à la relecture d’un passage déjà visité.
- `(fx: glitch)` / `(fx: glitch++)` — effet glitch plein écran une fois ; `(fx: glitch off)` l’arrête. `(fx: invert true)` / `(fx: invert false)` — inversion des couleurs (souvent avec le glitch). Sur une **option**, `(fx: …)` peut suivre le lien : `[libellé](id)(fx: glitch)`.
- `$texte$` — effet glitch sur un mot ou une expression (pas de Markdown à l’intérieur).
...
**Conditions** (dans `(if:…)(…)` / `(ifnot:…)(…)`) : `variable = valeur` ; raccourci `variable valeur` (littéral `true` / `false` / nombre / mot sans espace) ; ou un seul identifiant (truthy). Dans une condition composée, `once` signifie « passage cible pas encore visité » (ex. `(if: x = true & once)([…](id))`).

- **`once` (options)** : `(once)([libellé](id_passage))`, préfixe `once [libellé](id)`, ou atome `once` dans une condition — le choix n’apparaît plus après la première visite du passage cible.
